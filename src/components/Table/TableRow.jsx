...
const TableRow = ({
  ...
  const renderPurchasePrice = () => {
    ...
    // ✨ CORREGIDO: Usar purchasePriceLoading para controlar el estado de carga
    if (product.purchasePriceLoading) {
      return <span style={{ color: "#6c757d" }}>Cargando...</span>;
    }

    // Si no está cargando pero no hay precio, mostrar mensaje diferente
  ...
  return (
    <tr style={styles.row}>
      <td style={{ ...styles.cell, ...styles.codeCell }}>{product.sku}</td>
      <td style={{ ...styles.cell, ...styles.nameCell }}>{product.name}</td>
      ...
      <td style={{ ...styles.cell, ...styles.unitCell }}>
        {product.unit_of_measure || product.unit || product.unit_name || "N/A"}
      </td>
      ...
      <td style={{ ...styles.cell, ...styles.marginCell }}>{renderMargin()}</td>
      <td
        style={{
          ...styles.cell,
          ...(isDescriptionExpanded
