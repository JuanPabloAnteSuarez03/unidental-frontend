 ...
 */
export const getCreditAccountsWithWhatsApp = async (
    params = {},
    authToken
) => {
    if (!authToken) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            let mockAccounts = [
                // CUENTAS VENCIDAS
                {
                    ...
                    status: "vencido",
                    has_phone: true,
                    whatsapp_url: "https://wa.me/56956789123?text=Estimado%20Juan%20Pérez%2C%20le%20recordamos%20que%20tiene%20una%20cuenta%20pendiente%20de%20%24150%2C000%20vencida%20hace%2015%20días.",
                    whatsapp_message: "Estimado Juan Pérez, le recordamos que tiene una cuenta pendiente de $150,000 vencida hace 15 días. Por favor, póngase en contacto con nosotros para coordinar el pago."
                },
                {
                    id: 2,
                    ...
                    status: "vencido",
                    has_phone: true,
                    whatsapp_url: "https://wa.me/56987654321?text=Estimada%20María%20González%2C%20le%20recordamos%20que%20tiene%20una%20cuenta%20pendiente%20de%20%2485%2C000%20vencida%20hace%207%20días.",
                    whatsapp_message: "Estimada María González, le recordamos que tiene una cuenta pendiente de $85,000 vencida hace 7 días. Por favor, póngase en contacto con nosotros para coordinar el pago."
                },
                {
                    id: 3,
                ...
                // CUENTAS PRÓXIMAS A VENCER
                {
                    ...
                    status: "proximo",
                    has_phone: true,
                    whatsapp_url: "https://wa.me/56912345678?text=Estimada%20Ana%20López%2C%20le%20recordamos%20que%20tiene%20una%20cuenta%20de%20%24120%2C000%20próxima%20a%20vencer%20en%202%20días.",
                    whatsapp_message: "Estimada Ana López, le recordamos que tiene una cuenta de $120,000 próxima a vencer en 2 días. Por favor, esté atenta a la fecha de vencimiento."
                },
                {
                    id: 5,
                    ...
                    status: "proximo",
                    has_phone: true,
                    whatsapp_url: "https://wa.me/56945123678?text=Estimado%20Pedro%20Silva%2C%20le%20recordamos%20que%20tiene%20una%20cuenta%20de%20%2475%2C000%20próxima%20a%20vencer%20en%205%20días.",
                    whatsapp_message: "Estimado Pedro Silva, le recordamos que tiene una cuenta de $75,000 próxima a vencer en 5 días. Por favor, esté atento a la fecha de vencimiento."
                },
                // CUENTAS ACTIVAS (para include_all)
                {
                    ...
                    status: "activo",
                    has_phone: true,
                    whatsapp_url: "https://wa.me/56923456789?text=Estimada%20Laura%20Morales%2C%20le%20informamos%20sobre%20su%20cuenta%20activa%20de%20%24180%2C000.",
                    whatsapp_message: "Estimada Laura Morales, le informamos sobre su cuenta activa de $180,000. Su próximo vencimiento es el 1 de marzo."
                },
                {
                    id: 7,
                ...
                },
                {
                    ...
                    status: "activo",
                    has_phone: true,
                    whatsapp_url: "https://wa.me/56934567891?text=Estimada%20Sofia%20Vargas%2C%20le%20informamos%20sobre%20su%20cuenta%20activa%20de%20%24320%2C000.",
                    whatsapp_message: "Estimada Sofia Vargas, le informamos sobre su cuenta activa de $320,000. Su próximo vencimiento es el 1 de abril."
                }
            ];
            ...
            if (params.include_all) {
                ...
                mockAccounts = mockAccounts.filter(account => {
                    ...
                    if (params.include_upcoming && account.status === 'proximo') {
                ...
                const mode = params.include_upcoming ? 'Vencidos + Próximos a vencer' : 'Solo vencidos';
            ...
            console.log("🔧 Usando datos de prueba temporales para cuentas de crédito");
            console.log(`📊 Total de cuentas filtradas: ${mockAccounts.length}`);
            return mockData;
        }
        ...
        console.error("Error fetching overdue credit accounts with WhatsApp:", error);
        throw error;
    }
};
...
export const calculateCreditAccountStats = (accounts) => {
    ...
    const stats = {
        ...
        total_amount: accounts.reduce((sum, account) => sum + parseFloat(account.remaining_amount || 0), 0),
        overdue_count: accounts.filter(account => account.status === 'vencido').length,
        upcoming_count: accounts.filter(account => account.status === 'proximo').length,
        active_count: accounts.filter(account => account.status === 'activo').length,
        with_phone_count: accounts.filter(account => account.has_phone).length,
        urgent_count: accounts.filter(account => account.status === 'vencido' && account.days_overdue >= 15).length,
    };
...
export const filterCreditAccounts = (accounts, filters) => {
    ...
    if (filters.onlyWithPhone) {
        filteredAccounts = filteredAccounts.filter(account => account.has_phone);
    }
    ...
    if (filters.urgencyFilter && filters.urgencyFilter !== 'all') {
        filteredAccounts = filteredAccounts.filter(account => {
            const urgencyClass = getUrgencyClass(account.days_overdue, account.status);
    ...
    if (filters.statusFilter && filters.statusFilter !== 'all') {
        filteredAccounts = filteredAccounts.filter(account => account.status === filters.statusFilter);
    }
    ...
    if (filters.minAmount && filters.minAmount.trim()) {
        ...
        if (!isNaN(minAmount)) {
            filteredAccounts = filteredAccounts.filter(account => 
                parseFloat(account.remaining_amount) >= minAmount
            );
        }
    }
