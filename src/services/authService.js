  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorData.password)
        fieldErrors.push(`Contraseña: ${errorData.password.join(", ")}`);
...
export const requestPasswordReset = async (email) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const detail =
        errorData.detail ||
        (errorData.email ? `Email: ${errorData.email.join(", ")}` : null);
      throw new Error(
        detail ||
...
export const resetPasswordConfirm = async (uid, token, newPassword) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const fieldErrors = [];
      if (errorData.uid) fieldErrors.push(`UID: ${errorData.uid.join(", ")}`);
      if (errorData.token)
