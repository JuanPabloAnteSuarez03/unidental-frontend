...
const SalesPage = () => {
    ...
    const handleSubmitSale = async () => {
        ...
        if (paymentMethod === "credit" && !selectedCustomer) {
            alert(
                "Para ventas a crédito es obligatorio seleccionar un cliente."
            );
            return;
        }
        ...
        if (paymentMethod === "credit" && !creditConfig.isValid) {
            alert(
                "Por favor complete correctamente la configuración del crédito."
            );
            return;
        }
        ...
        try {
            ...
            console.log(
                "Método de pago a enviar (paymentMethod):",
                paymentMethod
            );
            const fixedPaymentMethod =
                paymentMethod === "cash" ? "normal" : paymentMethod;
            console.log(
                "Método de pago corregido (fixedPaymentMethod):",
                fixedPaymentMethod
            );
            const saleData = {
            ...
            if (paymentMethod === "credit") {
                try {
                    ...
                    console.log(
                        "🔍 DEBUG - Configuración de crédito:",
                        creditConfig
                    );
                    ...
                    if (
                        ...
                        const creditData = {
                            ...
                            initial_payment:
                                creditConfig.initialPayment.toString(),
                            installments_count: parseInt(
                                creditConfig.installmentsCount
                            ),
                        ...
                        console.log(
                            "  - Monto original:",
                            creditData.original_amount
                        );
                        console.log(
                            "  - Pago inicial:",
                            creditData.initial_payment
                        );
                        console.log(
                            "  - Cuotas:",
                        ...
                        const creditResponse =
                            await createCreditWithInstallments(
                                creditData,
                                authToken
                            );
                        console.log(
                            "✅ Crédito con pago inicial creado:",
                            creditResponse
                        );
                    } else {
    ...
    const handleConfirmConversion = async (
        ...
        try {
            ...
            if (
                selectedSuggestion.from_product.requires_batch_control &&
                selectedBatch
            ) {
            ...
            if (conversionResult.batch_info) {
                ...
                successMessage += `\n📅 Fecha de vencimiento heredada: ${new Date(
                    conversionResult.batch_info.expiry_date
                ).toLocaleDateString()}`;
            ...
            if (error.status === 400 && error.raw && error.raw.batch) {
                alert(
                    `❌ Error de lote: ${
                        error.raw.batch[0] ||
                        "Problema con el lote seleccionado"
                    }`
                );
            } else {
        ...
        try {
            ...
            if (paymentMethod === "credit") {
                try {
                    ...
                    console.error(
                        "❌ Error al crear crédito después de conversión:",
                        creditError
                    );
                    alert(
                        `⚠️ Venta registrada pero hubo un error al crear el crédito: ${creditError.message}`
                    );
                }
            }
            ...
            if (
                ...
                retryError.raw &&
                retryError.raw.error &&
                retryError.raw.error.suggestions
            ) {
    ...
    const handleGenerateQuote = () => {
        if (saleItems.length === 0) {
            alert(
                "Por favor agregue al menos un producto para generar una cotización."
            );
            return;
        }
                            ...
                            <div style={{ marginTop: "20px" }}>
                                ...
                                >
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setShouldInvoice(e.target.checked)
                                        }
