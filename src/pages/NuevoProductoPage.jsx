...
const NuevoProductoPage = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "purchase_price" || name === "sale_price") {
      ...
      const salePrice =
        parseFloat(name === "sale_price" ? value : formData.sale_price) || 0;
      ...
      while (attempts < maxAttempts) {
        ...
        if (result.valido && result.disponible) {
          ...
          if (!nextSku) {
            ...
            setSkuValidation({
              valid: false,
              message: "Formato de SKU inválido para auto-incremento",
            });
            setNotification({
              show: true,
              type: "error",
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      setNotification({
        show: true,
        type: "error",
        message: "Los productos compuestos deben tener al menos un componente",
      });
      setTimeout(() => {
        setNotification({ show: false, type: "", message: "" });
      }, 5000);
