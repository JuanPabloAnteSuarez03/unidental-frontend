...
const AnalisisPreciosPage = () => {
  ...
  const fetchData = async (
    ...
    try {
      ...
      if (!finalUrl) {
        ...
        finalUrl = BASE_URL + (params.length ? "?" + params.join("&") : "");
      }
  ...
  const filterProductsLocally = (searchTerm, categoryFilter = []) => {
    ...
    if (categoryFilter && categoryFilter.length > 0) {
      filtered = filtered.filter((item) => {
        ...
        return itemCategoryId && categoryFilter.includes(itemCategoryId);
      });
    }
  ...
  useEffect(() => {
    if (authToken && viewMode === "otros") {
      if (globalSearchTerm.trim()) {
        ...
        console.log("🔄 Restaurando datos originales de proveedores...");
  ...
  const loadGlobalSearchData = async () => {
    ...
    if (allSupplierProducts.length > 0) {
      console.log("📱 Usando búsqueda local en productos de proveedores...");
      performLocalSearch(globalSearchTerm);
      return;
    }
  ...
  const fetchAllSupplierOptions = async (supplierId) => {
    ...
    while (nextUrl && pageCount < maxPages) {
      pageCount++;
      console.log(`📄 Cargando página ${pageCount} de opciones de compra...`);
      ...
      console.log(`✅ Página ${pageCount}: ${data.length} productos cargados`);
      allData.push(...data);
  ...
  const loadSupplierData = async (supplierId, side) => {
    ...
    const setLoadingState = side === "left" ? setLeftLoading : setRightLoading;
    const setTableData = side === "left" ? setLeftTableData : setRightTableData;
  ...
  const performLocalSearch = (searchTerm) => {
    ...
    console.log("🔍 Realizando búsqueda local en productos de proveedores...");
    console.log("Término de búsqueda:", searchTerm);
    console.log("Filtro de categoría activo:", selectedCategory[0] || "Todas");

    // Obtener todos los productos de los proveedores seleccionados
    const allSupplierProducts = [...originalLeftData, ...originalRightData];

    if (allSupplierProducts.length === 0) {
      console.log("⚠️ No hay productos de proveedores cargados para buscar");
      setLocalSearchResults([]);
      return;
    }
    ...
    const results = allSupplierProducts.filter((product) => {
      ...
      const productName =
        getFieldValue(product, "product_name", ["name", "product_name"]) || "";
      const productSku = getFieldValue(product, "sku", ["product_sku"]) || "";
      const productDescription =
        getFieldValue(product, "description", ["product_description"]) || "";
  ...
  useEffect(() => {
    if (authToken && viewMode === "otros") {
      ...
      if (globalSearchTerm.trim()) {
        console.log("🔍 Reaplicando búsqueda con nuevo filtro de categoría...");
        performLocalSearch(globalSearchTerm);
      }
    }
  }, [
  ...
  useEffect(() => {
    if (
      ...
      getCategories(authToken)
        .then((cats) => {
          console.log(`✅ Categorías cargadas: ${cats.length} categorías`);
          setCategories(cats);
        })
        .catch((error) => {
      ...
      <div style={{ marginBottom: 24, display: "flex", gap: 12 }}>
        <button
          ...
          style={{
            ...
            boxShadow: viewMode === "tabla" ? "0 2px 8px #1976d233" : "none",
          }}
        >
        ...
        <button
          ...
          style={{
            ...
            boxShadow: viewMode === "otros" ? "0 2px 8px #1976d233" : "none",
          }}
        >
        ...
        <>
              ...
              <div style={{ position: "relative", width: "100%" }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    color: "#22292f",
                    outline: "none",
                    boxShadow: searchInput ? "0 2px 8px #1976d233" : "none",
            ...
            >
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (/^\d*\.?\d*$/.test(val)) setMinPrice(val);
                  }}
                  placeholder="Mínimo"
                  style={{
                    padding: "12px 14px",
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) => {
                    const val = e.target.value;
                    if (/^\d*\.?\d*$/.test(val)) setMaxPrice(val);
                  }}
                  placeholder="Máximo"
                  style={{
                    padding: "12px 14px",
          ...
          <div style={{ overflowX: "auto" }}>
                ...
                <tbody>
                  {loading ? (
                    ...
                    finalFilteredData.map((row, idx) => {
                      const productName =
                        row.product_name || row.nombre || row.name || "N/A";
                      const supplier =
                        row.supplier_name || row.supplier || "N/A";
                      ...
                      return (
                        <tr
                          key={idx}
                          style={{
                            backgroundColor: idx % 2 === 0 ? "#fff" : "#f6f8fa",
            ...
            >
              ...
              >
                Página {currentPage} de {totalPages} ({finalFilteredData.length}{" "}
                resultados)
              </span>
              <div
                style={{
      ...
      {viewMode === "otros" && (
        <div style={{ marginBottom: 24 }}>
          ...
          >
            ...
            >
              Busca productos por nombre entre los proveedores seleccionados
            </p>
            <div
              style={{
                display: "flex",
                ...
                >
                  ...
                  <input
                    ...
                    style={{
                      ...
                      cursor: suppliersLoading ? "not-allowed" : "text",
                    }}
                  />
                </div>
              </div>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              {selectedCategory.length > 0 && (
                ...
                >
                  Categoría activa:{" "}
                  <strong>
                    {categories.find((c) => c.id === selectedCategory[0])
          ...
          >
            <div style={{ flex: 1, minWidth: 320 }}>
              ...
              <select
                value={leftSupplier}
                onChange={(e) => setLeftSupplier(e.target.value)}
                disabled={suppliersLoading}
                style={{
                  padding: "12px 16px",
                  borderRadius: 8,
                  ...
                  fontSize: 16,
                  minWidth: 220,
                  background: leftSupplier ? "#e3f2fd" : "#f8fafc",
                  color: leftSupplier ? "#1976d2" : "#333",
                  fontWeight: leftSupplier ? 700 : 400,
                  outline: "none",
                  marginBottom: 8,
                  boxShadow: leftSupplier ? "0 2px 8px #1976d233" : "none",
                  transition: "all 0.2s",
                  opacity: suppliersLoading ? 0.7 : 1,
                  cursor: suppliersLoading ? "not-allowed" : "pointer",
                }}
              >
                <option value="">
                  {suppliersLoading
              ...
              <SupplierComparisonTable
                ...
                loading={leftLoading}
                supplierName={
                  suppliers.find((s) => s.id === parseInt(leftSupplier))
                    ?.name || ""
                }
                side="left"
            ...
            <div style={{ flex: 1, minWidth: 320 }}>
              ...
              <select
                value={rightSupplier}
                onChange={(e) => setRightSupplier(e.target.value)}
                disabled={suppliersLoading}
                style={{
                  padding: "12px 16px",
                  borderRadius: 8,
                  ...
                  fontSize: 16,
                  minWidth: 220,
                  background: rightSupplier ? "#e3f2fd" : "#f8fafc",
                  color: rightSupplier ? "#1976d2" : "#333",
                  fontWeight: rightSupplier ? 700 : 400,
                  outline: "none",
                  marginBottom: 8,
                  boxShadow: rightSupplier ? "0 2px 8px #1976d233" : "none",
                  transition: "all 0.2s",
                  opacity: suppliersLoading ? 0.7 : 1,
                  cursor: suppliersLoading ? "not-allowed" : "pointer",
                }}
              ...
              <SupplierComparisonTable
                ...
                supplierName={
                  suppliers.find((s) => s.id === parseInt(rightSupplier))
                    ?.name || ""
                }
                side="right"
                getFieldValue={getFieldValue}
