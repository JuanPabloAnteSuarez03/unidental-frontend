...
const OrdenesDeCompraPage = () => {
  ...
  useEffect(() => {
    ...
    getAllSuppliers(authToken)
      .then((allSuppliers) => {
        console.log(`✅ Total de proveedores cargados: ${allSuppliers.length}`);
  ...
  const total = orderItems.reduce(
    (sum, item) => sum + parseFloat(item.purchase_price || 0) * item.quantity,
    0
  );
    ...
    try {
      ...
      if (shouldGenerateOrder) {
        setOrderModalData({
          ...
          locationData: locations.find((loc) => loc.id === selectedLocation),
      ...
      setSubmitError(`Error al crear la orden de compra: ${error.message}`);
    } finally {
  ...
  const fetchOrders = async (page = 1) => {
    ...
    try {
      ...
      setOrdersTotalPages(
        data.count ? Math.max(1, Math.ceil(data.count / ORDERS_PAGE_SIZE)) : 1
      );
    } catch (error) {
  ...
  const handleCloseOrderModal = () => {
    ...
    if (orderModalData) {
      setSubmitSuccess(
        `✅ Orden de compra creada exitosamente!\n\n📋 Número de orden: ${
          orderModalData.orderData.order_number || orderModalData.orderData.id
        }\n🏢 Proveedor: ${orderModalData.supplierData.name}\n📦 Productos: ${
  ...
  const handleStatusChange = async (orderId, action) => {
    ...
    try {
      const actionConfig = pendingOrderActions.find((a) => a.value === action);
      if (!actionConfig) {
        ...
        prevOrders.map((order) =>
          order.id === orderId
            ? {
                ...
                status_display:
                  updatedOrder.status_display || actionConfig.label,
              }
            : order
        )
      );
      ...
      setSubmitError(`❌ Error al actualizar el estado: ${error.message}`);
    } finally {
  ...
  const redirectToStockMovements = async (orderId) => {
    ...
    try {
      ...
      const stockMovementData = {
        ...
        location: orderDetails.destination || orderDetails.destination_name,
            ...
            ? orderDetails.items.map(async (item, index) => {
                  ...
                  try {
                    ...
                    if (productResponse.ok) {
                      ...
                      requiresBatchControl =
                        productDetails.requires_batch_control || false;
                      console.log(
                ...
                return {
                  ...
                  product: {
                    ...
                    name: productName,
                    sku: productSku,
                    requires_batch_control: requiresBatchControl,
                  },
                  quantity: requiresBatchControl ? "" : quantity, // Si requiere lotes, no pre-llenar cantidad principal
      ...
      console.log("🔍 Datos finales para movimientos:", stockMovementData);
          ...
          style={{
            ...
            border:
              activeSection === "registro"
                ? "2px solid #2c3e50"
                : "2px solid #e3eaf3",
            background: activeSection === "registro" ? "#2c3e50" : "#fff",
            color: activeSection === "registro" ? "#fff" : "#2c3e50",
            fontWeight: 700,
            fontSize: "16px",
            cursor: "pointer",
            transition: "all 0.2s",
          ...
          style={{
            ...
            border:
              activeSection === "otra"
                ? "2px solid #2c3e50"
                : "2px solid #e3eaf3",
            background: activeSection === "otra" ? "#2c3e50" : "#fff",
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {order.supplier_details?.name ||
                                order.supplier_name ||
                                order.supplier ||
                                "Sin proveedor"}
                            </strong>
                          </div>
                          {order.supplier_details?.contact_name && (
                            <div
                              style={{
                                fontSize: "12px",
                                color: "#6c757d",
                            ...
                            >
                              Contacto: {order.supplier_details.contact_name}
                            </div>
                          )}
                        </td>
                        <td
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {order.destination_details?.name || "Sin destino"}
                            </strong>
                          </div>
                          {order.destination_details?.address && (
                            <div
                              style={{
                                fontSize: "12px",
                                color: "#6c757d",
                          ...
                          {order.status === "pending" ? (
                            ...
                            >
                              ...
                              {showStatusDropdown === order.id && (
                                ...
                                >
                                  {pendingOrderActions.map((action) => (
                                    <button
                                      key={action.value}
                                      onClick={() =>
                                        handleStatusChange(
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {order.created_by_username || "Usuario"}
                            </strong>
                          </div>
                          {order.created_at && (
                            <div
                        ...
                        >
                          <button
                            ...
                            style={{
                              padding: "8px 16px",
                              backgroundColor: isLoadingReceipt
                                ? "#95a5a6"
                                : "#3498db",
                              color: "white",
                              border: "none",
                              ...
                              alignItems: "center",
                              gap: "6px",
                              opacity: isLoadingReceipt ? 0.7 : 1,
                            }}
                          >
                            {isLoadingReceipt ? (
                              <svg
                            ...
                            {isLoadingReceipt ? "Cargando..." : "Ver Recibo"}
                          </button>
                        </td>
                      </tr>
                    ))}
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: ordersPage === 1 ? "#f8f9fa" : "#fff",
                      color: "#2c3e50",
                      fontWeight: 600,
                      fontSize: "14px",
                  ...
                  <button
                    ...
                    style={{
                      padding: "8px 16px",
                      backgroundColor: ordersPage === 1 ? "#e9ecef" : "#3498db",
                      color: ordersPage === 1 ? "#6c757d" : "white",
                      border: "none",
                      borderRadius: "6px",
                      cursor: ordersPage === 1 ? "not-allowed" : "pointer",
                      transition: "all 0.2s ease",
                  ...
                  <button
                    ...
                    style={{
                      padding: "8px 16px",
                      backgroundColor:
                        ordersPage === ordersTotalPages ? "#e9ecef" : "#3498db",
                      color:
                        ordersPage === ordersTotalPages ? "#6c757d" : "white",
                      border: "none",
                      borderRadius: "6px",
                      cursor:
