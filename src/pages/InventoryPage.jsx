...
const InventoryPage = () => {
    ...
    const { loadAllProducts } = useProducts(); // Importar función del contexto de productos
    // Estado para el modal de conversión global
    const [showGlobalConversionModal, setShowGlobalConversionModal] = React.useState(false);
    ...
    const [isSubmittingConversion, setIsSubmittingConversion] = React.useState(false);
    ...
    return (
        <>
            ...
            <Modal isOpen={showGlobalConversionModal} onClose={() => setShowGlobalConversionModal(false)}>
              <h2>Crear conversión de productos</h2>
              <p>Transforma un producto en otro (por ejemplo, caja a blister, blister a pastilla, etc).</p>
              <button
                onClick={async () => {
                ...
                style={{ marginBottom: 10, background: '#00b894', color: 'white', border: 'none', borderRadius: 6, padding: '6px 16px', fontWeight: 600, cursor: 'pointer', transition: 'background 0.2s' }}
                onMouseOver={e => e.currentTarget.style.background = '#019267'}
              ...
              <div style={{ margin: '16px 0' }}>
                ...
                <ProductSearchSelector
                  ...
                  onProductSelected={prod => setConversionForm(f => ({ ...f, from_product: prod }))}
                  placeholder="Buscar producto origen por nombre, SKU o código..."
                  minSearchLength={2}
                  maxResults={10}
                />
                {conversionForm.from_product && (
                  <div style={{ marginTop: 8, color: '#2c3e50', border: '1px solid #27ae60', borderRadius: 8, padding: 8, background: '#eafaf1', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ flex: 1 }}>
                      <strong>{conversionForm.from_product.name}</strong><br />
                      SKU: {conversionForm.from_product.sku}<br />
                      <span style={{ color: '#888', fontSize: 13 }}>Unidad: {conversionForm.from_product.unit} | Categoría: {conversionForm.from_product.category_name || ''}</span>
                    </div>
                    <button onClick={() => setConversionForm(f => ({ ...f, from_product: null }))} style={{ background: '#ff7675', color: 'white', border: 'none', borderRadius: 4, fontWeight: 700, fontSize: 18, width: 32, height: 32, cursor: 'pointer' }}>×</button>
                  </div>
                )}
                ...
                {conversionForm.from_product && conversionForm.to_product && (
                  <div style={{ marginTop: 18 }}>
                    <label style={{ fontWeight: 600 }}>
                      ¿Cuántas <span style={{ color: '#0984e3' }}>{conversionForm.to_product.unit}</span> de <span style={{ color: '#0984e3' }}>{conversionForm.to_product.name}</span> salen de 1 <span style={{ color: '#27ae60' }}>{conversionForm.from_product.unit}</span> de <span style={{ color: '#27ae60' }}>{conversionForm.from_product.name}</span>?
                    </label>
                    <input
                      type="number"
                      min={1}
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
                <button
                  onClick={async () => {
                    if (!conversionForm.from_product || !conversionForm.to_product || !conversionForm.conversion_rate) return;
                    ...
                    try {
                      const resp = await fetch("https://unidental-backend.onrender.com/api/catalogs/product-conversions/", {
                        ...
                        body: JSON.stringify({
                          from_product: conversionForm.from_product.id,
                  ...
                  style={{
                    background: isSubmittingConversion ? '#b2bec3' : '#0984e3',
                    color: 'white',
                    border: 'none',
                    borderRadius: 6,
                    padding: '8px 20px',
                    fontWeight: 600,
                    fontSize: 16,
                    cursor: isSubmittingConversion ? 'not-allowed' : 'pointer',
                    transition: 'background 0.2s',
                    opacity: isSubmittingConversion ? 0.7 : 1
                  }}
                  onMouseOver={e => { if (!isSubmittingConversion) e.currentTarget.style.background = '#0652DD'; }}
                  onMouseOut={e => { if (!isSubmittingConversion) e.currentTarget.style.background = '#0984e3'; }}
                >
                  {isSubmittingConversion ? 'Creando...' : 'Crear conversión'}
                </button>
                <button onClick={() => setShowGlobalConversionModal(false)} style={{ background: '#636e72', color: 'white', border: 'none', borderRadius: 6, padding: '8px 20px', fontWeight: 600, fontSize: 16, cursor: 'pointer' }}>Cerrar</button>
              </div>
            </Modal>
            <div
