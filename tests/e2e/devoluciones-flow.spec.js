...
test.describe("Flujo de Devoluciones - Crítico", () => {
    ...
    test("Debe procesar devolución de producto compuesto completa", async ({
        page,
    }) => {
        ...
        const nuevaDevolucionBtn = page
            .locator(
                'button:has-text("Nueva Devolución"), button:has-text("Crear"), button:has-text("Registrar")'
            )
            .first();
        await nuevaDevolucionBtn.click();

        // PASO 1: Buscar venta original
        const ventaInput = page
            .locator(
                'input[placeholder*="venta"], input[placeholder*="factura"], input[name*="sale"]'
            )
            .first();
        ...
        const productoCompuesto = page
            .locator(
                "text=/kit.*dental/i, text=/pack.*vitaminas/i, .composite-product"
            )
            .first();
        ...
        const cantidadDevolver = page
            .locator('input[name*="cantidad"], input[type="number"]')
            .first();
        ...
        const motivoSelect = page
            .locator('select[name*="motivo"], select[name*="reason"]')
            .first();
        ...
        const descomponerCheck = page
            .locator(
                'input[type="checkbox"]:near(text="descomponer"), input[name*="decompose"]'
            )
            .first();
        ...
        const procesarBtn = page
            .locator(
                'button:has-text("Procesar"), button:has-text("Confirmar"), button:has-text("Registrar")'
            )
            .first();
        ...
        const exitoDescomposicion = page
            .locator(
                "text=/producto.*descompuesto/i, text=/componentes.*devueltos/i, .success-message"
            )
            .first();
        ...
        const componentes = page
            .locator(".component-item, .individual-product")
            .count();
    ...
    test("Debe manejar devolución parcial de productos compuestos", async ({
        page,
    }) => {
        ...
        const productoKit = page
            .locator("text=/kit.*completo/i, .composite-product")
            .first();
        ...
        const exitoParcial = page
            .locator(
                "text=/devolución.*parcial/i, text=/2.*unidades.*devueltas/i"
            )
            .first();
    ...
    test("Debe validar stock al procesar devolución", async ({ page }) => {
        ...
        const errorValidacion = page
            .locator(
                "text=/cantidad.*excede/i, text=/no.*suficiente/i, .error-message"
            )
            .first();
    ...
    test("Debe generar reembolso automático", async ({ page }) => {
        ...
        const reembolsoCheck = page
            .locator(
                'input[type="checkbox"]:near(text="reembolso"), input[name*="refund"]'
            )
            .first();
        ...
        await page.waitForTimeout(2000);
        const reembolsoExitoso = page
            .locator("text=/reembolso.*generado/i, text=/monto.*devuelto/i")
            .first();
        ...
        const montoReembolso = page
            .locator('text=/$[d,.]+/, [data-testid="refund-amount"]')
            .first();
    ...
    test("Debe manejar devolución con componentes dañados", async ({
        page,
    }) => {
        await expect(page.locator("h1")).toContainText(/devoluc/i);
        ...
        const componentesDañados = page
            .locator('input[name*="damaged"], textarea[name*="observ"]')
            .first();
        if (await componentesDañados.isVisible()) {
            await componentesDañados.fill(
                "Componente A dañado, resto en buen estado"
            );
        }

        // Seleccionar disposición de componentes
        const disposicionSelect = page
            .locator('select[name*="disposition"], select[name*="disposal"]')
            .first();
        ...
        await page.waitForTimeout(2000);
        const procesamientoMixto = page
            .locator(
                "text=/componentes.*procesados/i, text=/parcialmente.*reutilizable/i"
            )
            .first();
    ...
    test("Debe actualizar inventario tras devolución", async ({ page }) => {
        ...
        const stockInicial = page
            .locator('[data-testid="product-stock"], .stock-count')
            .first();
        let stockAntes = 0;
