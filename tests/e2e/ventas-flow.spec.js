...
test.describe("Flujo de Ventas - Crítico", () => {
    ...
    test("Debe completar venta con cliente específico", async ({ page }) => {
        ...
        const sedeButton = page
            .locator(
                'button:has-text("Farmacia Central"), button:has-text("Sede Principal")'
            )
            .first();
        ...
        const cantidadInput = page
            .locator(
                'input[type="number"], input[name*="cantidad"], input[placeholder*="cantidad"]'
            )
            .first();
        ...
        const totalElement = page
            .locator(
                'text=/total.*$[d,.]+/i, [data-testid="total"], .total-amount'
            )
            .first();
        ...
        const procesarBtn = page
            .locator(
                'button:has-text("Registrar Venta"), button:has-text("Procesar"), button:has-text("Finalizar")'
            )
            .first();
        ...
        const exitoMessage = page
            .locator(
                "text=/venta.*exitosa/i, text=/registrada.*correctamente/i, .success-message"
            )
            .first();
    ...
    test("Debe manejar venta sin cliente específico", async ({ page }) => {
        ...
        const sinClienteBtn = page
            .locator(
                'button:has-text("sin cliente"), button:has-text("Sin Cliente")'
            )
            .first();
        ...
        const productoInput = page
            .locator('input[placeholder*="producto" i]')
            .first();
        ...
        const procesarBtn = page
            .locator(
                'button:has-text("Registrar"), button:has-text("Procesar")'
            )
            .first();
        ...
        const resultado = page
            .locator("text=/venta/i, text=/registr/i, text=/éxito/i")
            .first();
    ...
    test("Debe validar stock insuficiente", async ({ page }) => {
        ...
        await page.waitForTimeout(1000);
        const errorStock = page
            .locator(
                "text=/stock.*insuficiente/i, text=/sin.*stock/i, .error-message"
            )
            .first();
    ...
    test("Debe procesar venta con múltiples productos", async ({ page }) => {
        ...
        await page.waitForTimeout(2000);
        const exito = page
            .locator("text=/venta.*registrada/i, text=/éxito/i")
            .first();
