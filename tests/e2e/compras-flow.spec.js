...
test.describe('Flujo de Compras - Crítico', () => {
  ...
  test('Debe crear orden de compra completa', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2')).toContainText(/orden.*compra|compras/i);

    // Crear nueva orden
    const nuevaOrdenBtn = page.locator('button:has-text("Nueva Orden"), button:has-text("Crear"), button:has-text("Agregar")').first();
    await nuevaOrdenBtn.click();

    // PASO 1: Seleccionar proveedor
    const proveedorSelect = page.locator('select[name*="proveedor"], select[name*="supplier"], input[placeholder*="proveedor"]').first();
    ...
    if (await proveedorSelect.getAttribute('tagName') === 'SELECT') {
      ...
      const proveedorOption = page.locator('text=Farmacéutica ABC').first();
    ...
    const productoInput = page.locator('input[placeholder*="producto"], input[placeholder*="buscar"]').first();
    ...
    const cantidadInput = page.locator('input[type="number"], input[name*="cantidad"]').first();
    if (await cantidadInput.isVisible()) {
      await cantidadInput.fill('50');
    }

    // Establecer precio unitario
    const precioInput = page.locator('input[name*="precio"], input[placeholder*="precio"]').first();
    ...
    const totalElement = page.locator('text=/total.*\$|total.*:.*[\d,.]/, [data-testid="total"]').first();
    if (await totalElement.isVisible()) {
      await expect(totalElement).toBeVisible();
    }

    // PASO 4: Confirmar orden
    const confirmarBtn = page.locator('button:has-text("Confirmar"), button:has-text("Crear Orden"), button:has-text("Registrar")').first();
    await confirmarBtn.waitFor({ state: 'visible' });
    await confirmarBtn.click();

    // PASO 5: Verificar orden creada
    await page.waitForTimeout(2000);
    const exitoMessage = page.locator('text=/orden.*creada/i, text=/orden.*registrada/i, .success-message').first();
  ...
  test('Debe recibir productos de orden de compra', async ({ page }) => {
    ...
    const ordenPendiente = page.locator('tr:has-text("Pendiente"), .order-item:has-text("Pendiente")').first();
    if (await ordenPendiente.isVisible()) {
      await ordenPendiente.click();
    }

    // Acceder a recepción
    const recibirBtn = page.locator('button:has-text("Recibir"), button:has-text("Recepción")').first();
    if (await recibirBtn.isVisible()) {
      await recibirBtn.click();
    }

    // Confirmar cantidad recibida
    const cantidadRecibida = page.locator('input[name*="recibida"], input[name*="cantidad"]').first();
    ...
    const estadoSelect = page.locator('select[name*="estado"], select[name*="condition"]').first();
    if (await estadoSelect.isVisible()) {
      await estadoSelect.selectOption('bueno');
    }

    // Confirmar recepción
    const confirmarRecepcionBtn = page.locator('button:has-text("Confirmar Recepción"), button:has-text("Recibir")').first();
    ...
    await page.waitForTimeout(1500);
    const exito = page.locator('text=/recibido.*correctamente/i, text=/recepción.*exitosa/i').first();
  ...
  test('Debe manejar orden con múltiples productos', async ({ page }) => {
    ...
    const nuevaOrdenBtn = page.locator('button:has-text("Nueva"), button:has-text("Crear")').first();
    await nuevaOrdenBtn.click();

    // Seleccionar proveedor
    const proveedorSelect = page.locator('select, input[placeholder*="proveedor"]').first();
    if (await proveedorSelect.getAttribute('tagName') === 'SELECT') {
      await proveedorSelect.selectOption({ index: 1 });
    }

    // Agregar primer producto
    let productoInput = page.locator('input[placeholder*="producto"]').first();
    ...
    const confirmarBtn = page.locator('button:has-text("Confirmar"), button:has-text("Crear")').first();
    if (await confirmarBtn.isVisible()) {
      await confirmarBtn.click();
    }

    await page.waitForTimeout(2000);
    const exito = page.locator('text=/orden.*creada/i, text=/éxito/i').first();
    await expect(exito).toBeVisible();
  });
  ...
  test('Debe validar datos obligatorios en orden', async ({ page }) => {
    ...
    await page.waitForTimeout(1000);
    const errorValidacion = page.locator('text=/proveedor.*requerido/i, text=/seleccionar.*proveedor/i, .error-message').first();
    if (await errorValidacion.isVisible()) {
      await expect(errorValidacion).toBeVisible();
    }
  });

  test('Debe calcular correctamente el total de la orden', async ({ page }) => {
    ...
    const productoInput = page.locator('input[placeholder*="producto"]').first();
    ...
    await page.waitForTimeout(500);
    const totalCalculado = page.locator('text=/\$150\.00/, text=/150/, [data-testid="total"]').first();
